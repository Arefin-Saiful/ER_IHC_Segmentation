...
CKPT_DIR = PHASE2_ROOT / "outputs/checkpoints/transunet_style_120"
METRIC_DIR = PHASE2_ROOT / "outputs/metrics/transunet_style_120"
FIG_DIR = PHASE2_ROOT / "outputs/figures/transunet_style_120"
...
def build_2d_sincos_pos_embed(channels, height, width, device):
    """
    Dynamic 2D sine-cosine positional encoding.
    This avoids fixed positional embeddings, so the model works for both
    320x320 training crops and 512x512 full-validation images.
    """
    if channels % 4 != 0:
        raise ValueError("Transformer channel dimension must be divisible by 4.")

    y_embed = torch.linspace(0, 1, steps=height, device=device)
    x_embed = torch.linspace(0, 1, steps=width, device=device)
    yy, xx = torch.meshgrid(y_embed, x_embed, indexing="ij")

    dim_each = channels // 4
    omega = torch.arange(dim_each, device=device).float()
...
class TransformerBottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        pos = build_2d_sincos_pos_embed(c, h, w, x.device)
        tokens = tokens + pos
...
class TransUNetStyle(nn.Module):
    """
    Lightweight 2D TransUNet-style baseline:
    CNN encoder + transformer bottleneck + U-Net decoder.

    This is used as a transformer-based comparative baseline for 2D ER-IHC
    segmentation, not as a claimed novel method.
    """
    def __init__(
        self,
        in_channels=3,
    ...
    def forward(self, x):
        ...
        # Compatible with imported training loop.
        # aux_weight is 0.0, so repeated logits do not affect optimization.
...
def main():
    ...
    run_name = (
        f"phase2_transunet_style120_fullval_fold{args.fold}"
        f"_aug-{args.aug_mode}"
        f"_base{args.base_ch}"
        f"_crop{args.crop_size}"
        f"_tl{args.transformer_layers}"
        f"_h{args.transformer_heads}"
    )
    ...
    print("=" * 90)
    print("Phase 2D: TransUNet-style training for 120 epochs")
    print("=" * 90)
    ...
    summary = {
        ...
        "phase": "Phase 2D",
        "model": "TransUNet-style 120 epochs",
        "run_name": run_name,
        "fold": args.fold,
    ...
    summary_path = REPORT_DIR / f"28_phase2_transunet_style120_{run_name}_summary.json"
    ...
    print()
    print("=" * 90)
    print("Phase 2D TransUNet-style 120 training completed")
    print("=" * 90)
    print(f"Fold: {args.fold}")
