...
FOLDWISE_PATH = PROJECT_ROOT / "outputs/tables/results/deeplabv3_all_models_foldwise_metrics.csv"
...
COMPARISONS = [
    ("Proposed AMC-Ordinal", "ResUNet-DS"),
    ("Proposed AMC-Ordinal", "U-Net"),
    ("Proposed AMC-Ordinal", "DeepLabV3-ResNet50"),
    ("ResUNet-DS", "DeepLabV3-ResNet50"),
    ("ResUNet-DS", "U-Net"),
]
...
def main():
    print("=" * 90)
    print("Final statistical testing for all models")
    print("=" * 90)

    if not FOLDWISE_PATH.exists():
        raise FileNotFoundError(f"Missing foldwise result file: {FOLDWISE_PATH}")
    ...
    for model_a, model_b in COMPARISONS:
        ...
        for metric in METRICS:
            ...
            rows.append({
                ...
                "direction_score": direction_score(diff),
                "fold_differences": "; ".join([f"fold{fold}:{value:.5f}" for fold, value in zip(common_folds, diff)]),
            })

    stats = pd.DataFrame(rows)

    stats_path = OUT_DIR / "all_final_models_statistical_tests.csv"
    manuscript_stats_path = MANUSCRIPT_TABLE_DIR / "all_final_models_statistical_tests.csv"

    stats.to_csv(stats_path, index=False)
    stats.to_csv(manuscript_stats_path, index=False)

    key = stats[
        (stats["comparison"].isin([
            "Proposed AMC-Ordinal vs ResUNet-DS",
            "Proposed AMC-Ordinal vs DeepLabV3-ResNet50",
            "ResUNet-DS vs DeepLabV3-ResNet50",
        ]))
        & (stats["metric"].isin([
            "mean_dice_no_bg",
            "minority_dice_c2_c3_c4",
    ...
    key_path = OUT_DIR / "all_final_models_key_statistical_tests.csv"
    manuscript_key_path = MANUSCRIPT_TABLE_DIR / "all_final_models_key_statistical_tests.csv"
    ...
    report_path = REPORT_DIR / "11_all_final_models_statistical_testing_summary.json"
